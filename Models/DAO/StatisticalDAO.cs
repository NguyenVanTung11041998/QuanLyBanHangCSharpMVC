...
namespace Models.DAO
{
    public class StatisticalDAO : BaseDAO
    {
        ...
        {
            var prices = DBContext.Bills.Where(x => x.CreationTime.Year == year).GroupBy(x => x.CreationTime.Month).Select(x => new PriceDto { Month = x.Key, TotalPrice = x.Sum(p => p.TotalPrice) }).ToList();
